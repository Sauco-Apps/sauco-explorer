	...
	describe('GET /api/getPriceTicker', () => {
		it('should be ok', (done) => {
			getPriceTicker((err, res) => {
				...
				node.expect(res.body).to.have.deep.property('tickers.SAUCO.BTC').to.be.at.least(0);
				node.expect(res.body).to.have.deep.property('tickers.SAUCO.EUR').to.be.at.least(0);
				node.expect(res.body).to.have.deep.property('tickers.SAUCO.USD').to.be.at.least(0);
				// node.expect(res.body).to.have.deep.property('tickers.SAUCO.CNY').to.be.at.least(0);
