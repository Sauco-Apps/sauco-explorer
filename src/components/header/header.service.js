...
const HeaderConstructor = function ($rootScope) {
	...
	this.updatePriceTicker = (res) => {
		...
		if ($rootScope.currency.symbol !== 'SAUCO' &&
			(!$rootScope.currency.tickers ||
			!$rootScope.currency.tickers.SAUCO ||
			!$rootScope.currency.tickers.SAUCO[$rootScope.currency.symbol])) {
			$rootScope.currency.symbol = 'SAUCO';
		}
	};
